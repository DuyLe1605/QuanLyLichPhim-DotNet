...
class Program
{
    static void Main()
    {
        string filePath = @"d:\Study\.Net\BaiTapLon\BaiTapLon\Forms\Admin\Customers\UcCustomerManagement.cs";
        string content = File.ReadAllText(filePath);
        
        int startIdx = content.IndexOf("private Control CreateDetailPanel()");
        if (startIdx < 0) { Console.WriteLine("Cannot find start"); return; }
        startIdx = content.LastIndexOf('\n', startIdx) + 1;
        
        int endIdx = content.IndexOf("return panel;", startIdx);
        if (endIdx < 0) { Console.WriteLine("Cannot find end"); return; }
        
        endIdx = content.IndexOf("}", endIdx) + 1;

        string newCode = @"    private Control CreateDetailPanel()
    {
        ...
        content = content.Remove(startIdx, endIdx - startIdx).Insert(startIdx, newCode);
